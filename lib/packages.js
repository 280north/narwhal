...
exports.load = function (packagePrefixes) {
    ...
    analysis.plugins.forEach(function(plugin) {
        system.log.debug("Loading plugin: "+plugin);
        try {
            require(plugin);
        } catch (e) {
            system.log.warn("Error loading plugin: "+e);
        }
    });
...
exports.analyze = function analyze(analysis, catalog) {
    ...
    catalog.forEach(function (packageData) {
        ...
        // add any plugins to analysis
        if (packageData.plugins)
            analysis.plugins.unshift.apply(analysis.plugins, packageData.plugins);
    });
};
