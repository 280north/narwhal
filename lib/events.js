...
Q.Signal.constructor = function (action, Event) {
    var self = this;
    ...
    self.observe = function (observer, Event) {
     ...
     *
     */
    self.observer = function (observer, Event) {
        var that = this;
        return function () {
            var args = arguments;
...
Q.Emitter.constructor = function () {
    var self = this;

    var signals = {};

    self.constructObservable = function (observable) {
    ...
    self.getObservers = function (name) {
     ...
     */
    self.setSignal = function (name, signal, Event) {
        if (Object.prototype.hasOwnProperty.call(signals, name))
            throw new Error("Signal already set: " + name);
...
Q.Observable.constructor = function () {
    var self = this;
     ...
     */
    self.setSignal = function (name, observer, Event) {
        self[name] = self.setSignalRaw(name, observer, Event).emitter();
    };
...
Q.Deferred.constructor = function (canceller) {
    var self = this;
     ...
     */
    self.setSignal("progress", Object.freeze(function (_progress) {
        progress = _progress;
    }));
 ...
 */
Q.Deferred.prototype.timeout = function (time) { // NodeJS
    var self = this;
...
// Dojo
Q.Deferred.prototype.addCallback = function (observer) {
    Q.when(this.promise, observer);
    return this;
};

// Dojo
Q.Deferred.prototype.addBoth = function (callback, errback) {
    Q.when(this.promise, callback, errback);
    return this;
};

Q.Deferred.prototype.toSource =
Q.Deferred.prototype.toString = function () {
...
Q.Promise.constructor = function (canceller) {
    var self = this;
    ...
    self.emit = Object.freeze(function (operator /* ...args */) {
        var method = self[operator] || function () {
            if (!self.noSuchMethod)
                throw new Error("Promise did not provide requisite noSuchMethod method");
            return self.noSuchMethod.apply(self, [operator].concat(arguments));
        };
        var args = Array.prototype.slice.call(arguments, 1);
        Q.enqueue(function () {
 ...
 */
Q.Promise.prototype.noSuchMethod = function () {
    return Q.Rejection("No such method");
};

Q.Promise.prototype.toSource =
Q.Promise.prototype.toString = function () {
...
Q.Promise.prototype.addCallback = function (observer) {
    this.observe("ok", observer);
    return this;
};

// NodeJS, Dojo (on a Deferred)
Q.Promise.prototype.addErrback = function (observer) {
    this.observe("error", observer);
    return this;
};

// Dojo (on a Deferred)
Q.Promise.prototype.addBoth = function (callback, errback) {
...
// NodeJS
Q.Promise.prototype.addCancelback = function (cancelback) {
...
Q.Rejection = function (reason) {
    var self = this instanceof Q.Rejection ? this : Object.create(Q.Rejection.prototype);
    Q.Promise.call(self);

    self.setSignal("noSuchMethod", function (block) {
        return block ? block(Q.Rejection(reason)) : Q.Rejection(reason);
    });
...
Q.Rejection.prototype.toString = function () {
    return "[object Promise rejected]";
};
...
Q.Reference.constructor = function (object) {
    var self = this;
    ...
    self.near = self.valueOf; // ref_send

    self.setSignal("when", function (block) { // ref_send 
        return block ? block(object) : object;
    });
    self.then = self.when; // Kris Zyp's CommonJS proposal
 ...
 */
Q.when = function (value, ok, error, progress) {
    return Q.valueOf(Q.whenPromise(value, ok, error, progress));
};
 ...
 */
Q.whenPromise = function (value, ok, error, progress) {
    var deferred = Q.Deferred();
    var done = false; // sensure the untrusted promise
    ...
    Q.promise(value).emit("when", function (value) {
        ...
        deferred.resolve(error ? error(reason) : Q.Rejection(reason));
    }, deferred.progress);
 ...
 */
Q.put = function (object, name, value) {
