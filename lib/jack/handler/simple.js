...
Simple.run = function(app, options) {
    ...
    if (typeof Packages.org.simpleframework.http.connect.SocketConnection === "function")
        connection = new Packages.org.simpleframework.http.connect.SocketConnection(handler);
    else if (typeof Packages.org.simpleframework.transport.connect.SocketConnection === "function")
...
Simple.process = function(app, request, response) {
    ...
    var output = response.getPrintStream(); //getOutputStream();
