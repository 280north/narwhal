...
Jack.Handler.Servlet.prototype.process = function(request, response) {
    ...
    // set the headers
    headers.each(function(key, values) {
        var items = [];
        values.each(function(v) { items.push(v); });
    ...
    var output = response.getOutputStream(),
        chunked_response = (headers.includes('transfer-encoding') && headers.get('transfer-encoding') !== 'identity');
