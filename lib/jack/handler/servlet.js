...
Servlet.prototype.process = function(request, response) {
    ...
    env["jack.input"]           = new JavaIO(request.getInputStream(), null);
...
var JavaIO = function(inputStream, outputStream) {
    this.inputStream = inputStream;
    this.outputStream = outputStream;
    this.buffer = null;
}
JavaIO.prototype = new IO();

JavaIO.prototype.read = function(length) {
    if (!this.buffer || length > this.buffer.length)
        this.buffer = java.lang.reflect.Array.newInstance(java.lang.Byte.TYPE, length);
    
    var bytes = this.inputStream.read(this.buffer, 0, length)
    
    if (bytes !== length)
        log.debug("not enough to read (requested="+length+", actual="+bytes+")");
        
    return String(new java.lang.String(this.buffer, "US-ASCII")); // eek?!
}
