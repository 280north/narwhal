...
Response.prototype.setCookie = function(key, value) {
    ...
    if (typeof value === "object") {
        if (value.domain) domain = "; domain=" + value.domain ;
        if (value.path) path = "; path=" + value.path;
        if (value.expires) expires = "; expires=" + value.expires.toGMTString();
        if (value.secure) secure = "; secure";
        if (value.httponly) httponly = "; HttpOnly";
        value = value.value;
    }
    ...
    cookie += domain + path + expires + secure + httponly;
