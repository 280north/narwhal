...
Jack.Lint.prototype.call = function(env) {
    if (!env)
    ...
    this.check_status(result.status);
    this.check_headers(result.headers);
    this.check_content_type(result.status, result.headers);
    this.check_content_length(result.status, result.headers, env);
...
Jack.Lint.prototype.check_headers = function(headers) {
    ...
    if (typeof headers.each !== "function")
        throw new Error("headers object should respond to #each, but doesn't");
...
Jack.Lint.prototype.check_content_length = function(status, headers, env) {
    var chunked_response = (headers.includes('transfer-encoding') && headers.get('transfer-encoding') !== 'identity');
