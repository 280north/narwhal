...
Jack.Lint = function(app) {
    return function(env) {
        return (new Jack.Lint.Context(app)).run(env);
    }
}

Jack.Lint.Context = function(app) {
    this.app = app;
}

Jack.Lint.Context.prototype.run = function(env) {
    if (!env)
...
Jack.Lint.Context.prototype.forEach = function(block) {
...
Jack.Lint.Context.prototype.close = function() {
    if (this.body.close)
        return this.body.close();
}

Jack.Lint.Context.prototype.check_env = function(env) {
    if (env && typeof env !== "object" || env.constructor !== Object)
        throw new Error("env is not a hash");
    
    ["REQUEST_METHOD","SERVER_NAME","SERVER_PORT","QUERY_STRING","jack.version","jack.input","jack.errors","jack.multithread","jack.multiprocess","jack.run_once"].forEach(function(key) {
...
Jack.Lint.Context.prototype.check_input = function(input) {
    // FIXME:
...
Jack.Lint.Context.prototype.check_error = function(error) {
...
Jack.Lint.Context.prototype.check_status = function(status) {
    if (!(parseInt(status) >= 100))
        throw new Error("Status must be >=100 seen as integer");
}
Jack.Lint.Context.prototype.check_headers = function(headers) {
...
Jack.Lint.Context.prototype.check_content_type = function(status, headers) {
...
Jack.Lint.Context.prototype.check_content_length = function(status, headers, env) {
