...
parser.option('-r', '--require', 'module')
    ...
    .action(function (options, name, value) {
        options.todo.push(["require", value]);
    });
...
parser.option('-x', '--shebang', 'shebang')
    .help('skip the first line of the main module, allowing use of non-Unix #!shebangs.')
    .set(true)
    .hidden();
...
parser.option('-V', '--version')
    ...
    .action(function () {
        parser.print("Narwhal Version 0.");
        parser.exit();
    });
...
parser.option('--narwhal')
    ...
    .choices(['left', 'right'])
    .action(function (options, name, value) {
        print(exports[value.toUpperCase()]);
    })
    .halt()
    .hidden();
