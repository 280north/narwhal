require("jack/handler/jetty");
require("jack/commonlogger");

Roundabout = function(roads, options) {
...
Roundabout.prototype.invoke = function(env) {
    // push a new context
    var context = new Roundabout.Context(env, new Jack.Request(env), new Jack.Response());
    ...
    // otherwise use the response in the Roundabout context
        ...
        else if (context.body && context.body.each)
    ...
    if (roads) {    
        ...
        for (var i = 0; roads && i < roads.length; i++) {
            ...
            if ((wildcardMatches = path.match(regex)) && (!road.options.filter || road.options.filter.apply(context, [env, context]))) {
                ...
                if (typeof road.app == "function")
                    return road.app.apply(context, [env, context]);
                else
                    return road.app.invoke(env, context);
            }
        }
    }
...
Roundabout.FORWARD_METHODS.each(function(method) {
...
Roundabout.run = function() {
    Jack.Handler.Jetty.run(new Jack.CommonLogger(Roundabout.application));
}
