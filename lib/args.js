...
exports.Parser.prototype.printHelp = function (options) {
    ...
    var self = this;
    this.print(
        '\0bold(\0cyan(Usage:\0)\0) ' + system.fs.basename(options.command) + ' [OPTIONS]' + 
    ...
    if (this._help)
        self.print('\0cyan(' + this._help + '\0)');
    util.forEachApply(
        util.items(this._commands),
        function (name, command) {
            var parser = command[0];
            var handler = command[1];
            self.print('  \0blue(' + name + '\0)' + (
                parser._help ?
                ': \0purple(' + parser._help + '\0)':
                ''
            ));
        }
    );
    this._options.forEach(function (option) {
        ...
        if (option._help)
            message.push(': \0purple(' + option._help + '\0)');
        if (option._choices) {
            var choices = option._choices;
            if (!util.isArrayLike(choices))
                choices = util.keys(choices);
            message.push(' \0yellow((' + choices.join(', ') + ')\0)');
        }
        if (option._halt)
            message.push(' \0red((final option)\0)');
