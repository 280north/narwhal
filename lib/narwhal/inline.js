(function (path, bootstrap) {
    ...
    require.when = function (id, observer) {
        console.log(id + ' when');
        if (catalog[id] && catalog[id].ready)
            return observer();
        if (!observers[id])
            observers[id] = [];
    ...
    require.main = function (id) {
        console.log(id + ' main');
        return require.when(id, function () {
            require(id);
        });
    };
    ...
    var ready = false;
    var readyObservers = [];
    var whenReady = function (observer) {
        if (ready) {
            observer();
        } else {
    ...
    var sendReady = function () {
        while (readyObservers.length) {
            readyObservers.shift()();
        }
    };

    require.preload(bootstrap);
