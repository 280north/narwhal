...
exports.fnmatch = function(/*String*/pattern, /*String*/string, /*int*/flags)
{
    ...
    for ( var p = 0;  p < patternLength;  p++ )
    {
        ...
        switch (c)
        {
            ...
            if (flags & exports.FNM_ALTERNATION) {
                if (stringLength === n)
                    return exports.FNM_NOMATCH;
                ...
                while (true)
                {
                    // closing brace, done whether it's a match or not
                    if (c == '}')
                    {
                        break;
                    }
                    // comma, check to see if it's still a match
                    else if (c == ',')
                    {
                        // still a match so mark as complete
                        if (matched)
                            complete = true;
                        // not a match so reset "n" and continue
                        else
                            n = startN;
                        matched = true;
                    }
                    ...
                    {
                        // replace with next character if it's escaped, otherwise treat "\" as normal
                        if (c == '\\' && !(flags & exports.FNM_NOESCAPE))
                            c = fold(pattern.charAt(++p), flags);
