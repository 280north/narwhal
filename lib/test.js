...
parser.option('--quiet', 'quiet').def(false).set(true);
...
exports.Log.prototype.fail = function (exception) {
    ...
    this.print("\0yellow(FAIL" + (exception.message ? ":\0) " + exception.message : "\0)"));
    this.print("expected: "+jsDump.parse(exception.expected));
    this.print("actual: "+jsDump.parse(exception.actual));
    this.print("operator: \0red("+exception.operator+"\0)");
...
exports.Log.prototype.error = function (exception, message) {
    ...
    this.print("\0red(ERROR: \0)"+exception);
    if (stacktrace)
        this.print("\0blue("+stacktrace+"\0)");
...
exports.Log.prototype.report = function () {
    this.stream.stream.print(
        "\0green(Passes: " + this.passed + "\0), " +
        "\0yellow(Fails: " + this.failed + "\0), " +
        "\0red(Errors: " + this.errored + "\0)"
    );
};
